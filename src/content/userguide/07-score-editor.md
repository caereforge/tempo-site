...
The Score Editor is where you teach Tempo about a source. Open it and you can decide what every event from a given provider should look like: its color, its severity, the headline metric it shows, the actions it offers, and how it groups with related events. Every change autosaves to the score's JSON file, so the changes apply to the next event that arrives. No restart and no redeploy are required.
...
- **Try panel** on the right: picks a recent event from this provider and shows you what your current edits would render for it. Live preview as you edit
...
Edits apply automatically. Every change autosaves to the score's JSON file (roughly 400 ms after you stop typing) and the next event from this provider runs through the updated score. There is no Save button and no Discard button.

> 💡 **Note**: edits persist across app restarts and across Tempo updates. Tempo writes the saved version to disk, and you can always recover a bundled score's factory state. See [§7.8](#78---persistence-and-recovery).

### The score chip bar

The chip bar at the top is your score selector. It lists one chip per installed score, sorted by display name; each chip shows the provider's display name and color dot, click one to switch. Edits to one score are scoped to that score; switching scores flushes the pending autosave first, so you never lose an in-flight edit (no save-or-discard prompt).

To create a score from scratch, add a JSON file to the Scores folder (`~/Library/Application Support/Tempo/Scores/`); it appears as a new chip once the file watcher picks it up. This is covered in [§11 - Score authoring](/docs/11-score-authoring). The **Open score location in Finder** button lives in the **Source** tab's toolbar (it reveals the score's JSON in that folder), and the helper/README links live in the Source tab's helper section where the source ships one. The chip bar itself is only a score selector.
...
- **Trash icon** removes a rule. There is no in-editor undo; if you remove the wrong one, re-add it. For a bundled score, you can also recover the shipped version (see [§7.8](#78---persistence-and-recovery))
...
The Stack grouping section configures whether and how related events from this provider collapse into stacks (see [§5.4 - Stacked events](/docs/05-event-panel#54---stacked-events)). It covers template grouping (one or more key templates plus a time window), which is the grouping available today and the part you edit here. A richer session-based grouping (where a down/recovery cycle folds into one episode) is used internally by some bundled stateful scores but is not yet user-configurable; custom grouping support may arrive in a later version.
...
The Try panel on the right shows you a sample event from this provider and tells you what stack identity it would resolve to under your current edits. Useful when you're editing templates and want to confirm the resolution actually matches what you expected.
...
Edits autosave automatically. Roughly 400 ms after you stop typing, the change is written to disk, the file watcher picks it up, and the next event from this provider runs through the updated score. The Try panel updates immediately so you can confirm the result before traffic arrives. There is no Save button and no Discard button.
...
- **Open score location in Finder** in the Source tab toolbar opens `~/Library/Application Support/Tempo/Scores/` with the score selected
...
2. **Set the color**: click the color picker on the **Source** tab, pick a green that matches your aesthetic, or paste `#30D158` (the bundled default)
...
4. **Add a custom label**: click into the `outcome=ok` rule's "badge label" field, type `Backup OK · ${metadata.delta}`. The edit autosaves; the next ok event will render with `BACKUP OK · +147KB` on its severity pill
...
6. **Confirm in the Try panel**: the right-side panel shows a recent Kopia event with the current edit applied. If it looks right, you're done. If not, tweak and re-check

The whole loop is a handful of edits. No restart and no manual JSON editing are required.
