...
A related habit: when **two sources see the same device** (UniFi sees a TV as a network client, Home Assistant sees it as a media player), pick the one with the cleaner signal and let only that one report it. One device, one storyteller.
