...
Proxmox Backup Server 3.2 added **webhook notification targets** as a native transport. This score gives Tempo a place to receive those notifications and attaches 10 one-click actions on each event: open the PBS UI, jump into datastores or the task list, SSH to the server, tail the task log remotely, copy details for an incident ticket.
...
2. Double-click it. Tempo opens a review sheet, then click **Install**. The score is saved to `~/Library/Application Support/Tempo/Scores/proxmox-backup.tempo-score`.
