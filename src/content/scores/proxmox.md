...
2. Double-click it. Tempo opens a review sheet, then click **Install**. The score lands in `~/Library/Application Support/Tempo/Scores/proxmox.tempo-score`.
...
Trigger any notification. The easiest path is to run a manual `vzdump` backup on a small VM, or trigger the test action from **Datacenter → Notifications → tempo → Test** if your PVE version exposes it.
